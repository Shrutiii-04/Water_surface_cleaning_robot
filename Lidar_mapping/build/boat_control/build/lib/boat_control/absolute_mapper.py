...
class AbsoluteMapper(Node):
    def __init__(self):
        ...
        self.declare_parameter('map_size', 100.0)  # meters (square map)
        self.declare_parameter('origin_x', -50.0)  # map origin X
        self.declare_parameter('origin_y', -50.0)  # map origin Y
        ...
        self.declare_parameter('min_range', 1.5)  # ignore readings closer than this (filter boat/noise)
        self.declare_parameter('max_range', 30.0)  # ignore readings farther than this (filter max-range returns)
    ...
    def synced_callback(self, odom_msg: Odometry, scan_msg: LaserScan):
        ...
        with self.map_lock:
            ...
            for i, range_val in enumerate(scan_msg.ranges):
                ...
                # Use our stricter range limits to filter noise and self-readings
                # Also filter out readings very close to sensor max (likely "no return" values)
                if range_val < self.min_range or range_val > self.max_range:
                    angle += scan_msg.angle_increment
                    continue
                
                # Filter readings that are close to the sensor's max range (likely no-hit returns)
                if range_val > scan_msg.range_max * 0.95:
                    angle += scan_msg.angle_increment
                    continue
