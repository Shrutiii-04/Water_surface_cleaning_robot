#!/usr/bin/env python3
"""
Boat Teleop Control - Hold keys to move
Fixed for velocity-controlled thrusters with opposing prop axes.

Propeller joint axes:
  - Left propeller:  axis = +X (1, 0, 0)
  - Right propeller: axis = -X (-1, 0, 0)

When both props have POSITIVE angular velocity commands:
  - Left prop rotates CW around +X → pushes water FORWARD → boat moves BACKWARD
  - Right prop rotates CW around -X → pushes water BACKWARD → boat moves FORWARD

So for forward motion:
  - Left: POSITIVE velocity (push backward)  
  - Right: NEGATIVE velocity (push backward, but axis is flipped)
"""
    ...
    try:
        while rclpy.ok():
            ...
            if key:
                ...
                # No key pressed - stop if was moving
                if key_held:
                    node.stop()
                    key_held = False
