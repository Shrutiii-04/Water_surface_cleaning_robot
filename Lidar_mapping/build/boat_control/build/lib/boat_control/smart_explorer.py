...
class NavState(Enum):
    EXPLORING = 1       # Moving toward a goal
    AVOIDING = 2        # Obstacle avoidance maneuver
    ROTATING = 3        # Rotating to find clear path
    STUCK_RECOVERY = 4  # Stuck recovery
...
class SmartExplorer(Node):
    def __init__(self):
        ...
        self.declare_parameter('turn_gain', 25.0)         # How aggressively to turn toward goal
        self.declare_parameter('obstacle_distance', 4.0)  # Start avoiding at this distance
        self.declare_parameter('critical_distance', 2.0)  # Emergency stop/turn distance
        self.declare_parameter('goal_tolerance', 3.0)     # Close enough to goal
        self.declare_parameter('exploration_radius', 40.0) # How far to look for frontiers
        ...
        # LIDAR data - store as polar coordinates
        self.scan_ranges = []
        self.scan_angles = []
        ...
        self.goal_timeout = 30.0  # Switch goal if can't reach in 30s
        
        # Map data for frontier detection
        self.map_data = None
        self.map_info = None
        
        # Exploration pattern - use a spiral/lawnmower pattern as backup
    ...
    def generate_exploration_pattern(self):
        ...
        goals = []
        # Create a expanding square spiral pattern
        step = 8.0  # meters between waypoints
        max_dist = 35.0
        
        # Start with nearby goals, expand outward
        for radius in np.arange(step, max_dist, step):
            # Four corners of the square at this radius
    ...
    def select_goal(self):
        ...
        if frontiers:
            # Find the closest frontier that's in a new direction
            best_frontier = None
            ...
            for fx, fy in frontiers:
                ...
                # Skip if we've been there recently
                too_close_to_visited = False
                for vx, vy in self.goals_visited[-10:]:  # Last 10 goals
                ...
                # Score based on distance (prefer closer) and angle (prefer ahead)
                angle_to_frontier = math.atan2(fy - self.y, fx - self.x)
                angle_diff = abs(self.normalize_angle(angle_to_frontier - self.yaw))
                
                score = dist + angle_diff * 5.0  # Penalize goals behind us
        ...
        # Fall back to pattern exploration
    ...
    def do_exploring(self):
        ...
        # Calculate speed based on obstacle proximity and angle error
        ...
        # Calculate differential thrust for steering
        # Positive angle_error = goal is to the left = turn left = more right thrust
        turn_component = self.turn_gain * angle_error
        
        left_thrust = speed - turn_component
        right_thrust = speed + turn_component
        
        # Clamp to reasonable values (keep both positive for forward motion)
    ...
    def do_avoiding(self):
        ...
        # Turn away from obstacle while moving slowly forward
    ...
    def do_rotating(self):
        ...
        if state_time > 8.0:
            # Rotated a lot, try recovery
            self.state = NavState.STUCK_RECOVERY
            self.state_start_time = time.time()
            return
        
        # Rotate with one thruster
        if self.rotation_direction > 0:
    ...
    def do_stuck_recovery(self):
        ...
        if state_time < 2.0:
            ...
            # Done, try exploring again with new goal
